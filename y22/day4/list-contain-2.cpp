...
int partialOverlaps(vector<int> firstList, vector<int> secList){
    if(firstList[0] <= secList[0] && firstList[1] >= secList[0]){
        return 1;
    }
    else if(firstList[0] >= secList[0] && firstList[0] <= secList[1]){
        return 1;
    }
    return 0;
}
...
int main(){
    ...
    while(getline(cin, line)){
        ...
        res += partialOverlaps(first, second);
    }
    printf("Result: %d\n", res);
    return 1;
}